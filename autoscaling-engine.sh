...
function start {
    ...
    if [ "${screen_exists}" -eq 0 ]; then
        screen -c screenrc -d -m -S ${_screen_session_name} -t autoscaling java -jar "$_project_base/build/libs/$_process_name-$_version.jar" --spring.config.location=file:${_config_file}
    else
        screen -S ${_screen_session_name} -p 0 -X screen -t autoscaling java -jar "$_project_base/build/libs/$_process_name-$_version.jar" --spring.config.location=file:${_config_file}
    fi
}
