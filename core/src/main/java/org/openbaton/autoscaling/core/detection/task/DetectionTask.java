...
public class DetectionTask implements Runnable {
  ...
  public void run() {
    ...
    log.debug(
        "DetectionTask: Checking Alarms of AutoScalingPolicy "
    ...
    if (vnfr != null) {
      for (ScalingAlarm alarm : autoScalePolicy.getAlarms()) {
        ...
        log.trace(
            "DetectionTask: Measurement result on vnfr "
                + vnfr.getId()
                + " on metric "
                + alarm.getMetric()
                + " with statistic "
        ...
        if (detectionEngine.checkThreshold(
            alarm.getComparisonOperator(), alarm.getThreshold(), finalAlarmResult)) {
          alarmsWeightFired = +alarm.getWeight();
          log.debug(
              "DetectionTask: Alarm with id: "
                  + alarm.getId()
                  + " of AutoScalePolicy with id "
                  + autoScalePolicy.getId()
                  + " is fired");
        } else {
          log.trace(
      ...
      if (detectionEngine.checkThreshold(
        ...
        if (fired == false) {
          ...
          log.info(
              "Threshold of AutoScalingPolicy with id "
                  + autoScalePolicy.getId()
                  + " is not crossed anymore. This means that the Alarm is cleared -> "
                  + autoScalePolicy.getThreshold()
                  + autoScalePolicy.getComparisonOperator()
                  + finalResult);
          fired = false;
    ...
    log.trace(
        "DetectionTask: Starting sleeping period ("
            + autoScalePolicy.getPeriod()
            + "s) for AutoScalePolicy with id: "
            + autoScalePolicy.getId());
